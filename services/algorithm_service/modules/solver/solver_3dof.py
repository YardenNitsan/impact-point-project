...
from modules.atmosphere.isa import compute_isa_atmosphere_state, compute_speed_of_sound
...
# TEMP DATASET MODE:
# לא משתמשים כרגע ב-ISA כדי לבנות אטמוספירה לפי גובה
# from modules.atmosphere.isa import compute_isa_atmosphere_state, compute_speed_of_sound

from modules.atmosphere.isa import compute_speed_of_sound
from modules.atmosphere.era5_loader import sample_column_at_altitude
...
def derivatives(
    ...
    *,
    ...
    # temperature_K, pressure_Pa, _density = compute_isa_atmosphere_state(
    #     altitude_m,
    #     sea_level_temperature_K,
    #     sea_level_pressure_Pa,
    # )
    ...
    # a_sound = max(compute_speed_of_sound(temperature_K), MIN_SPEED_OF_SOUND_MPS)
        ...
        # TEMP DATASET MODE:
    # משתמשים ישירות בעמודת ERA5 המקומית לפי הגובה הנוכחי,
    # ולא ב-ISA שמחשב תנאים לבד.
    if era5_column is not None:
        atm = sample_column_at_altitude(era5_column, altitude_m)
        ...
        #     sea_level_pressure_Pa,
        # )
        raise RuntimeError("era5_column is required in TEMP dataset mode")
